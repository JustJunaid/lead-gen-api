...
def _make_session_maker():
    """Create a fresh async engine + session maker for the current event loop.

    Each Celery task runs in its own event loop (via run_async), so we can't
    reuse the shared async_engine from database.py — its connection pool would
    be tied to a previous (closed) loop.
    """
...
def verify_leads_batch(self, job_id: str):
    ...
    async def _process_job():
        engine, session_factory = _make_session_maker()
        try:
